<template>
    <div class="facebook">
        <h1>Twitter Service</h1>
        <input type="text" v-model="username" placeholder="Twitter account ..." required>
        <p>{{error}}</p>
...
export default {
    ...
    methods: {
        async sender () {
            const params = {'twitterAccount': this.username}
            await axiosGet.post('/twitter/', params)
            .then((response) => {
