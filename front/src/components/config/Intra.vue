<template>
    <div class="intra">
        <h1>Intranet Epitech</h1>
        <input @keyup.enter="sender()" v-model="autologin" placeholder="Auto login link ..." required>
        <p>{{error}}</p>
...
export default {
    ...
    methods: {
        async sender () {
            const params = {'autoToken': this.autologin}
            await axiosGet.post('/intra/', params)
            .then((response) => {
